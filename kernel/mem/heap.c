...
int liballoc_lock() {
    if (lock == 1) {
        kprintf("[x] liballoc lock is acquired!!!!!!\n");
        while (true);
    }
    lock = 1;
    return 0;
...
int liballoc_unlock() {
    if (lock != 1) {
        kprintf("liballoc locking inconsistency!\n");
    }
    lock = 0;
    return 0;
}
...
void __print_used_mem()
{
    kprintf("[*] Memory in use: %d bytes\nMemory allocated: %d\n", l_inuse, l_allocated);
    return;
}
...
static void liballoc_dump()
{
    ...
    kprintf( "liballoc: System memory allocated: %i bytes\n", l_allocated );
    kprintf( "liballoc: Memory in used (malloc'ed): %i bytes\n", l_inuse );
    kprintf( "liballoc: Warning count: %i\n", l_warningCount );
    kprintf( "liballoc: Error count: %i\n", l_errorCount );
...
#ifdef DEBUG
    kprintf( "liballoc: %x PREFIX(free)( %x ): ",
            __builtin_return_address( 0 ),
            ptr );
#endif
