...
int init_paging(uint32_t mem){
    kprintf("mm: available memory: %d KB\n", mem);
    set_nframes(mem / 4);
    return 0;
}
...
void free_frame(page_t *p) {
    if (!p->frame) {
        kprintf("Page at 0x%x attempted to free frame 0x%x\n", p, p->frame);
        while (true);
    }
...
void paging_fini()
{
    kprintf("mm: mmap reserved memory: %d KB\n", mem_used());
    ...
    kprintf("mm: page directory setup.\n");
    switch_page_dir(kernel_dir);

    kprintf("mm: total reserved memory: %d KB\n", mem_used());
}
